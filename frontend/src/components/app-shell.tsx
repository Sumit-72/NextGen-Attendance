...
export function AppShell({
  ...
  async function handleSignOut() {
    await signOut();
    router.push(role === "Teacher" ? "/teacher/login" : "/student/login");
  }
      ...
      <header className="sticky top-0 z-20 border-b border-(--border) bg-[color:rgba(255,255,255,0.66)] backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-6 py-4 md:px-10 lg:px-16">
          <Link href="/home" className="flex items-center gap-3 text-lg font-semibold tracking-wide">
            <span className="grid h-10 w-10 place-items-center rounded-2xl bg-[linear-gradient(135deg,var(--primary),var(--secondary))] text-sm font-bold text-white shadow-[0_10px_30px_rgba(15,118,110,0.24)]">
              AE
            </span>
            <span>
              AttendEdge
          ...
          <div className="flex flex-wrap items-center gap-3">
            <nav className="flex flex-wrap items-center gap-2 rounded-full border border-(--border) bg-white/70 p-2 text-sm text-(--muted) shadow-[0_10px_30px_rgba(15,23,32,0.06)]">
            ...
            {user ? (
              <div className="flex items-center gap-3 rounded-full border border-(--border) bg-white/80 px-4 py-2 text-sm shadow-[0_10px_30px_rgba(15,23,32,0.06)]">
                ...
                <button
                  ...
                  className="rounded-full border border-(--border) px-3 py-1.5 text-xs font-medium transition hover:bg-[var(--foreground)]/5"
                >
                  Sign out
                </button>
              </div>
